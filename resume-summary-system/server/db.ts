...
import { InsertResume, Resume, resumes, processingLogs, InsertProcessingLog, ProcessingLog } from "../drizzle/schema";
...
export async function createResume(data: InsertResume): Promise<Resume | null> {
  ...
  const created = await db.select().from(resumes).where(eq(resumes.id, id as number)).limit(1);
...
export async function updateResume(id: number, data: Partial<Resume>): Promise<Resume | null> {
  const db = await getDb();
  if (!db) return null;
